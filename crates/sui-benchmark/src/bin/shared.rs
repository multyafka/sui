...
enum NextOp {
    Stat(Instant),
    Request(Instant),
    Response(Option<(Instant, (ObjectID, (ObjectRef, Owner)))>),
}
...
async fn main() {
    ...
    let mut gas = vec![];
    let mut counters_gas = generate_gas_objects(num_shared_counters.try_into().unwrap());
    ...
    eprintln!("package ref = {}, {}", package_ref.0, package_ref.1);
    gas.pop();

    let qd_and_gas: Vec<_> = gas
        .into_iter()
        .map(|g| (quorum_driver_handler.clone_quorum_driver(), g))
        .collect();

    // create counters
    let futures = qd_and_gas.into_iter().map(|(qd, gas_object)| async move {
        let tx =
    ...
    eprintln!("Done creating {} counters!", counter_and_gas.len());

    let gas_per_worker = counter_and_gas.len() / opts.num_workers;
    let gas: Vec<Vec<(ObjectID, (ObjectRef, Owner))>> = counter_and_gas
        .chunks(gas_per_worker)
        .map(|s| s.into())
        .collect();
    let mut tasks = Vec::new();
    let (tx, mut rx) = mpsc::unbounded_channel::<Stats>();
    (0..opts.num_workers).for_each(|i| {
        let mut free_pool = gas[i].clone();
        ...
        let task = tokio::spawn(async move {
            ...
            futures.push(Box::pin(
                get_next_tick(Instant::now(), 1_000_000 as u64 / opts.target_qps)
                    .map(NextOp::Request),
            ));
            futures.push(Box::pin(
                get_next_tick(
                    Instant::now()
                        + Duration::from_micros(opts.stat_collection_interval * 1_000_000),
                    1_000_000 * opts.stat_collection_interval,
                )
                .map(NextOp::Stat),
            ));
                    ...
                    NextOp::Request(_) => {
                        if free_pool.is_empty() {
                            ...
                            futures.push(Box::pin(
                                get_next_tick(
                                    Instant::now()
                                        + Duration::from_micros(1_000_000 as u64 / opts.target_qps),
                                    1_000_000 as u64 / opts.target_qps,
                                )
                                .map(NextOp::Request),
                            ));
                            continue;
                        ...
                        let owner = gas.1 .1;
                        let tx = make_counter_increment_transaction(
                            gas.1 .0,
                            package_ref,
                            counter_id.clone(),
                        );
    ...
    tasks.push(tokio::spawn(async move {
        ...
        while let Some(s @ Stats {
            ...
            stdout.write_all(format!("Throughput = {}, min_latency_s = {}, max_latency_s = {}, num_success = {}, num_error = {}, no_gas = {}, submitted = {}, in_flight = {}", total_qps, min_latency.as_millis(), max_latency.as_millis(), num_success, num_error, num_no_gas, num_submitted, num_in_flight).as_bytes()).unwrap();
